...
class Jikexueyuan:
    def __init__(self):
        self.indexurl = "http://www.jikexueyuan.com/course/typescript/"
        self.videoIDs = []
        self.videoIndexUrls = []
        self.subjects = []
        self.courselinks = []
    ...
    def GetAllCourseUrl(self, html):
        soup = BeautifulSoup(html, 'html.parser')
        pattern = re.compile(u'\u2605')
        ul = soup.find("div", {"class":"lesson-list"})
        for di in ul.find_all("div",{"class":"lessonimg-box"}):
            self.courseurls.append(di.find("a")["href"])
    ...
    def run(self):
        targetpage = "http://www.jikexueyuan.com/course/android/?pageNum=3"

        #url = self.indexurl + str(classID) + ".html"
        #print (url)
        shtml = self.GetHtml(targetpage)
        self.GetAllCourseUrl(shtml)
        for url in self.courseurls:
            print ("url: " + url)
            thtml = self.GetHtml(url)
            #print (shtml)
            self.courselinks[:] = []
            self.title = ""
            self.subjects[:] = []
            self.GetAllVideoIDs(thtml)
            self.GetVideoUrl()
